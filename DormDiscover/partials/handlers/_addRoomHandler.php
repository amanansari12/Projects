...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($address) || empty($area_id) || empty($price) || empty($singleRoom) || empty($attachBathroom)) {
        ...
        header('Location: /findAccomodation/renting.php?formFill=false');
    } else {
        ...
        if ($query_result) {
            ...
            for ($i = 0; $i < count($_FILES['roomImages']['name']); $i++) {
                ...
                if ($uploadOk == 0) {
                    ...
                    if (move_uploaded_file($roomImages["tmp_name"][$i], $target_file)) {
                        // Store image path and room ID in the database (you need to implement this part)
                        // Example: $database->insertImage($room_id, $target_file);
            ...
            $showAlert = true;
            header('Location: /findAccomodation/renting.php?result=true');
        } else {
            header('Location: /findAccomodation/renting.php?result=false');
        }
    }
}
